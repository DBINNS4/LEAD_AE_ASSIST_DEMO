...
const runBlackFreezeDetect = (filePath) => {
  try {
    const args = [
      '-v', 'error',
      '-i', filePath,
      '-vf', 'blackdetect,freezedetect',
      // write any bytes somewhere harmless without using the null muxer
      '-f', 'md5', devNull
    ];
    ...
    if (result.error) {
      console.error('[FFmpeg Spawn Error]', result.error);
    }
    if (result.stderr) {
      console.error('[FFmpeg stderr]', result.stderr.toString());
    }
    console.log('[FFmpeg exited]', result.status);
    return result.stderr.toString().trim();
  } catch (err) {
    return `Error running black/freezedetect: ${err.message}`;
  }
};
  ...
  args.push('-profile:v', profileMap[outputFormat] || '3'); // default to HQ
    ...
    function runOne(inputPath, index, streamId = index) {
      return new Promise((resolveOne) => {
        ...
        const statusMap = { transcoded: false, verified: false, outputFile: outName };
        let qualityResult = { status: 'skipped', reason: 'not requested' };
        const args = buildCommand(inputPath, outPath, progressFile);
...
proc.on('exit', (code, signal) => {
  ...
  sendTranscodeLog(`🚪 FFmpeg exited (code ${code}, signal ${signal || 'none'})`);
        ...
        proc.on('close', async (code) => {
          ...
          if (code === 0) {
            logs.push(`✅ Done: ${path.basename(outPath)}`);
            sendTranscodeLog(`✅ Done: ${path.basename(outPath)}`);

            if (config.revealAfter) {
              if (process.platform === 'darwin') {
                spawn('open', ['-R', outPath], { detached: true });
              } else if (process.platform === 'win32') {
                spawn('explorer', [`/select,${outPath}`], { detached: true });
              }
            }

            if (config.openInMediaInfo) {
              if (process.platform === 'darwin') {
                spawn('open', ['-a', 'MediaInfo', outPath], { detached: true });
              } else if (process.platform === 'win32') {
                spawn('cmd', ['/c', 'start', '', 'MediaInfo.exe', outPath], { detached: true });
              }
            }

            if (config.queueForAutomation) {
              logs.push(`🧠 Queued for automation (not implemented yet).`);
              sendTranscodeLog(`🧠 Queued for automation (not implemented yet).`);
            }
          } else {
            logs.push(`❌ Failed: ${path.basename(outPath)} (code ${code})`);
            sendTranscodeLog(`❌ Failed: ${path.basename(outPath)} (code ${code})`);
            qualityResult = { status: 'skipped', reason: 'transcode failed' };
          }

  try {
    if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
      const okMsg = `📄 Output exists: ${path.basename(outPath)}`;
      logs.push(okMsg);
      ...
      try {
        ...
        logs.push(warn);
        sendTranscodeLog(warn);
      }


      const inMeta = await getFFprobeData(inputPath);
      const outMeta = await getFFprobeData(outPath);
      let resDiff = false;
      let rateDiff = false;
      if (inMeta && outMeta) {
      ...
      if (shouldDelete) {
        try {
          ...
          const delMsg = `🧹 Deleted incomplete file: ${path.basename(outPath)}`;
          logs.push(delMsg);
      ...
      if (verification?.detectBlack) {
        const detectOut = runBlackFreezeDetect(outPath);
        detectOut.split('\n').forEach(line => {
          if (line.trim()) {
            const msg = `🔍 ${line.trim()}`;
            logs.push(msg);
      ...
      const errMsg = `❌ Output missing or empty: ${path.basename(outPath)}`;
      logs.push(errMsg);
      sendTranscodeLog(errMsg);
      ...
      qualityResult = { status: 'error', reason: verErr.message };
          ...
          if (global.queue) {
            const statusText = code === 0
              ? `✅ Done: ${path.basename(outPath)}`
              : `❌ Failed: ${path.basename(outPath)}`;
    ...
    (async () => {
      ...
      logs.push(`📄 Files processed: ${completed} of ${total}`);
 
      const archivePath = archiveLog(logs, 'transcode');
