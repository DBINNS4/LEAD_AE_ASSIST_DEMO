...
function validateN8nUrl(n8nUrl, allowlist = []) {
  const trimmed = (n8nUrl || '').trim();
  if (!trimmed) {
    return {
      valid: false,
      message: '❌ Please provide an n8n URL when webhook logging is enabled.'
    };
  }
  ...
  try {
    ...
    return {
      valid: false,
      message: '❌ Invalid n8n URL. Please use a full http/https address.'
    };
  }
  ...
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    return {
      valid: false,
      message: '❌ n8n URL must start with http:// or https://.'
    };
  }
  ...
  if (isPrivateHostname(parsed.hostname)) {
    return {
      valid: false,
      message: '❌ n8n URL cannot target localhost or private networks.'
    };
  }
  ...
  if (normalizedAllowlist.length) {
    ...
    if (!match) {
      return {
        valid: false,
        message: `❌ n8n URL host not allowed. Allowed hosts: ${normalizedAllowlist.join(', ')}`
      };
    }
  }
...
async function runAdobeUtilities(config = {}) {
  ...
  log.push = (msg, detail = '', isError = false, fileId = '') => {
    ...
    // Auto-detect severity when not explicitly set
    const hasErrorMark = /❌|error/i.test(normalized);
    const hasWarnMark = /⚠️|warn/i.test(normalized);
    const level = isError
      ? 'error'
      : hasErrorMark
      ? 'error'
    ...
    jobLogger[level](normalized, meta);
  ...
  const pushLog = (msg, opts = {}) => {
    if (opts && typeof opts === 'object' && (opts.detail || opts.isError || opts.fileId)) {
      ...
      log.push(msg, detail, isError, fileId);
    } else {
      log.push(msg);
    }
  };
  ...
  if (validationErrors.length) {
    validationErrors.forEach(msg => log.push(msg, '', true));
  ...
  if (enableN8N) {
    if (n8nUrl) {
      const n8nValidation = validateN8nUrl(n8nUrl, n8nAllowlist);
      if (!n8nValidation.valid) {
        pushLog(n8nValidation.message || '⚠️ n8n URL blocked by validation.', {
          isError: true
        });
        pushLog('ℹ️ Skipping webhook trigger due to invalid URL.');
      }
      ...
      if (n8nValidation.valid) {
        pushLog(`🛰️ Preparing to send data to: ${n8nValidation.url}`);
