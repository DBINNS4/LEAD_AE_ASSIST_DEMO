...
async function runIngest(config) {
  ...
  log.push = (msg, detail = '', isError = false, fileId = '') => {
    sendLogMessage('ingest', msg, detail, isError, fileId);
    return origPush(msg);
  };
...
if (!destination || !destination.trim()) {
  log.push('❌ Ingest cancelled: No valid destination path provided.');
...
if (!fs.existsSync(destination)) {
  ...
  if (response === 0) {
    try {
      ...
      log.push(`❌ Failed to create destination: ${err.message}`);
...
if (backup && backupPath && !fs.existsSync(backupPath)) {
  ...
  if (response === 0) {
    try {
      ...
      log.push(`❌ Failed to create backup path: ${err.message}`);
    }
  } else {
...
function buildCopyTask(file) {
  ...
  return async (streamId) => {
    ...
    try {
      ...
      log.push(`❌ Error ingesting ${relPath}: ${err.message}`);
      counters.failed++;
...
if (!config.maxThreads || isNaN(config.maxThreads)) {
  try {
    ...
    log.push(`⚡ Estimated write speed: ${speed} MB/s`);
...
if (!watchMode) {
  const archivePath = archiveLog(log, 'ingest');
